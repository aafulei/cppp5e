...
`i != j < k` is equivalent to `i != (j < k)`. The subexpression `(j < k)` would evaluate to `1` if `j < k` or `0` if `j >= k`. Then the expression will compare `i` with the result, and yield `1` if the values are unequal, or `0` if the values are equal.
