...
int main()
{
	...
	// dynamic binding: read(), which accepts an &istream, now takes in an &istringstream
	// read(iss) returns a reference to iss;
	// iss.rdstate() returns an object of type istringstream::iostate
	// istringstream::goodbit is an iostate, whose value is guaranteed to have value 0
	// call operator () and dot operator (.) are in the same level of precedence
