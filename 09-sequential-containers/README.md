...
It is illegal, prior to C++17, because the order of evaluation is not guaranteed. We are not sure if the increment will take place before or after the assignment. However, since C++17 it has been guaranteed that every value computation and side-effect of the right-hand side of the assignment operator is sequenced before the left-hand side. Therefore, the call would become legal (though meaningless) in C++17.
